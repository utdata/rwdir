...
If you think you'll use a date parts more than once, then it makes sense to create new variables and save them. You might make several date parts, but we'll start with only one.
...
You can see the resulting new columns are real dates, but they have the "lowered" to the first day of the month or year:

- The `mo_floor` gives you the first day of the month for that `chart_date`.
- The `yr_floor` gives you the first day of the year for that `chart_date`.
...
Let's create our data with dates and save it. I'm making a "mutate machine" with a glimpse at the end so I can see all the new variables.

```{r}
hot100_dateparts <- hot100 |> 
  mutate(
    yr = year(chart_date),
    mo_label = month(chart_date, label = TRUE),
    mo_floor = floor_date(chart_date, unit = "month")
