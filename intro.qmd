...
![Posit Cloud Connect page](images/mls-analysis.png)

This website was created using Quarto and R, and the tool I used to write it was the RStudio IDE. Here's the crazy part: **I didn't have to write any HTML code,** I just wrote my thoughts in text and my code in R. With a few short lines of configuration and a click of a button I was able to publish my work on the Internet for free.
...
For this to work you need to first sign up for a free account with  [Posit's Connect Cloud](https://connect.posit.cloud/). (I think if you are using posit.cloud it will use that). The first time you go through this process it will also ask you to authorize the connection between RStudio and your browser. 

Also make sure you've Rendered your site at least once or you'll get an error saying that it "Could not find files to deploy" when you publish.

1. From within RStudio with a Quarto page open (perhaps your `index.qmd` file), click on the  **Publish** button that appears in the Quarto file tool bar on the far right.
1. A window will pop up asking where you want to publish to. Choose **Posit Connect Cloud**.
  ![Publish To options](images/publish-connect-choice.png)
1. On the next screen you'll be asked how you wish to publish your project. Choose **Publish finished website only**.
![](images/publish-website-only.png)
1. At this point you should see RStudio working to render and publish your project.
8. Once done, your browser should open to the  _Settings_ for that project. To get the PUBLIC url, click on the **Gear icon** to get the settings, then choose the **URL** option.
1. At that screen you can choose to customize your URL to make it a little less random, perhaps using your project slug that it starts with when you click the **Customize your URL** button. If you do that, you'll need to republish your site before using that new custom URL.

Here is a video of going through that whole process.

{{< video https://youtu.be/sBUyNf4MWOk >}}

A note about the video above: I said I _think_ I could republish only a selection of files to make corrections, but that is not true. Make sure you republish all your files to ensure your site works properly.
...
-   Re-publish your pages to Posit Connect Cloud.
