...
::: callout-warning
I think Quarto updates are done. 
:::
...
Before we dive into RStudio and programming and all that, I want to show you where we are heading, so you can "visualize success". As I wrote in the book intro, I'm a believer in Scripted Journalism ... data journalism that is repeatable, transparent and annotated. As such, the whole purpose of this book is to train you **how to create documents to share your work**.
...
This is a website with all the code from a data journalism project. If you click on the links for [Cleaning](https://christianmcdonald.quarto.pub/mls-salaries/01-cleaning.html) you can read where the data come from and see all the steps I went through -- with code and explantion -- to process the data so I could work with it. And in the [Analysis 2023](https://christianmcdonald.quarto.pub/mls-salaries/02-analysis.html) you'll see me set out with some questions I had for the data, and then the code to find those answers, along with the explanations of how and why I did it that way.

![Quarto Pub page](images/mls-analysis.png)

This website was created using Quarto and R, and RStudio was the tool I used to write the code. Here's the crazy part: **I didn't have to write any HTML code,** I just wrote thoughts in text and my code in R. With few short lines of configuration and a two-word command `quarto publish` I was able to publish my work on quarto.pub for free.

Keep this in mind:

- We can also easily publish our work in other formats, like PDF, Word or even a slide show.
...
Let's walk through the files here and explain what they are, though not in order shown.
...
- The `index.qmd` file **IS THE DOCUMENT YOU ARE WORKING FROM**. This is the "home page" of your website, and in this class you'll usually use it to describe a little about your assignment. You will always create and edit your documents in `.qmd` files, which stands for Quarto Markdown. (We might also encounter and edit `.Rmd` files, which are very similar, but just a little less awesome.) The `about.qmd` file is another Quarto document created as a placeholder. We'll usually rename/reuse that or delete it.
...
The big thing to remember is this: We edit files with the `.qmd` suffix.
...
A bunch of stuff will happen in your Terminal pane.
...
(@)  Follow the directions for your operating system linked below to create a compressed version of your `yourname-final-project` folder.
  a. [Compress files on a Mac](https://www.macinstruct.com/tutorials/how-to-compress-zip-files-and-folders-on-a-mac/).
  a. [Compress flies on Windows](https://www.laptopmag.com/articles/how-to-zip-files-windows-10).
