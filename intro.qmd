...
::: callout-warning
- This has been update for Quarto
- I could use a section that links to a finished project so students can understand where are going with all this.
:::
...
There are some preferences in RStudio that I would like you to change. By default, the program wants to save a the state of your work (all the variables and such) when you close a project, [but that is not good practice](https://www.r-bloggers.com/2017/04/using-r-dont-save-your-workspace/). We'll change that.
...
3.  For **Project Type**, choose **Quarto Project**.
...
![Quarto project name, directory](images/create-quarto-project.png)
...
Let's walk through w the files here and explain what they are.

- The `_quarto.yml` file is a YAML configuration file for your project. The file isn't really required for what we are doing here, and there isn't much inside of it now, but it allows us to apply Quarto settings across all your files. We might not get to much into that, but you can read more about it in the [Quarto Guide](https://quarto.org/docs/guide/) if you like.
- The `christian-first-project.qmd` file **IS THE DOCUMENT YOU ARE WORKING FROM**. You will always do your editing from  `.qmd` files, which stands for Quarto Markdown. (We might also encounter and edit `.Rmd` files, which are very similar, but just a little less awesome.)
...
So, out these files the one we edit is the Quarto document with the `.qmd` suffix. Our projects will end up with multiple Quarto files.
...
I could write reams of text about the evolution and birth of Quarto, but you won't really care. Let's break this down to what you need to know:
...
-   We "write" the document and can "run" the code to see the results, but all the while we are **building** documents that we will *rendered* to share with others. Our goal with these documents is to explain to an "audience" what are doing with our data analysis. Often our most important audience is our future self.

Think of *Rendering* like exporting or publishing a pretty version of your work.
...
Perhaps I have too much going on in that screenshot, but you'll be in this view a lot as you work with your code. You'll be writing and running code in your document at top left, and then periodically rendering it to see what the finished version looks like.
...
When we created our project, RStudio added the name of our folder here, **which kinda sucks** because that is not what the title is for. It should describe what we are doing like the top of a Microsoft Word or Google Docs document.

1.  Edit the text inside the quotes to be more like a title you would find in a Word or Google Doc document, like this:

``` txt
---
title: "Christian's First Quarto document"
---
```
...
- **Cmd+Return** will run a single line of code within a code block.
- **Cmd+Shift+Return** will run a whole code chunk.
:::

## Let's do some data analysis

Remember the goal of all our work here in this class is to explain to use data analysis to make sense of the world and then explain to others what we are doing. There are two parts to that:

- What are we thinking?
- Our code.

Our goal with this notebook is to discover what is the [socially-acceptable age to date someone older or younger than ourselves](socially-acceptable maximum age of anyone you should date). Yes, this is a Seinfeld episode come to life in a data class.
...
::: callout-note
I should note at this point there is a "Visual" editor where RStudio gives you more formatted look as your editing as it writes Markdown underneath the hood. I want you to use the "Source" editor so you can see and learn the underlying Markdown syntax. All my examples will be written in "Source" mode.
:::
...
Now you know the youngest a person should be that you date, FWIW. We don't recreate the assignment of the `age` variable since we already have it. A Quarto document is designed to run from top to bottom so that all the pieces work together.
...
Note there are a few new files there now:

- The `.html` file is the rendered version of your `.qmd` document.
- the `_files` folder holds other resources created during rendering.
...
3.  [Compress files on a Mac](https://www.macinstruct.com/tutorials/how-to-compress-zip-files-and-folders-on-a-mac/).
4.  [Compress flies on Windows](https://www.laptopmag.com/articles/how-to-zip-files-windows-10).
