...
library(tidyverse)
leso_sample <- read_rds("data-original/leso-sample.rds")
```
...
7.  Create two folders: `data-original` and `data-processed`.
...
1.  Use the `download.file()` function to download the date into your `data-original` folder. Remember you need three arguments:
...
-   It should be saved into your `data-original` folder with a name for the file.
...
Once you've built your code chunk and run it, you should make sure the file downloaded into the correct place: in your `data-original` folder.

::: callout-tip
If you get an error about the path, you might make sure you created the `data-original` folder first.
:::
...
1.  Use `read_csv()` to read the file from our `data-original` folder.
...
## Controlled vs. non-controlled {#controlled}
...
1. Start a new Markdown section that explains you are categorizing the controlled data.
2. Add a code chunk with the code below, and run it.
...
If you want to learn more about the code in that block, read the Appendix chapter titled [Categorization](categorization.qmd).
...
-   [`case_when()`](https://dplyr.tidyverse.org/reference/case_when.html) allows you to categorize a new column based on logic within your data. There is more about this in the [this appendix](categorization.qmd).
