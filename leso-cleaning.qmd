...
1.  For the project type, choose **Quarto Website**
1.  Name the directory "yourname-military-surplus", but with your name.
1.  In the `index.qmd` file, replace the title value with "Military Surplus", as that is our project title.
1.  Remove the boilerplate stuff after the YAML title and write in Markdown a little about what this is ... a class project about surplus military equipment. You can come back to this later to flesh it out.
...
4.  Open `_quarto.yml` and change the navigation link from `about.qmd` to `01-cleaning.qmd`.
...
2.  Add a code chunk formatted like below. I'll explain after. 
...
These two bits at the top starting with `#|` are special commands for the R chunk. The first word of each line is the command followed by a colon `:` and the bit after is the setting for that command.

- `#| label: setup`: The `label` is a name for the R chunk and `setup` is a super special label. 
    - Chunk labels are optional, but can be useful as they show up in your bookmarks. Some label names have special properties, like this one. A `setup` chunk will be run first no matter where we are in the notebook. Helpful to make sure your libraries are loaded.
- `#| message: false`: This option stops the message about all the different packages in the tidyverse when the library is loaded. It just cleans up the notebook.

One other note about this ... we should only need the `tidyverse` library for this notebook because the data already has clean names (no need for `janitor`) and the dates will import correctly (no need for `lubridate` for this notebook, but will will use it for our analysis).
...
In English, we are creating a new tibble `leso_tight` and then filling it with this: The `leso` data AND THEN we use select to remove the column called `sheet`. We then glimpse the new data.
...
In the last part of the chunk, we are taking `leso_total` object and we are peeking at some rows and some columns, but we are **not saving** those changes into a new object. There is no `<-` operator here. We are just printing the results to the screen so we can _see_ parts of the data more clearly.
...
In the Billboard lesson you used `filter()` to get No. 1 songs and to get a date range of data. We need to do something similar here to get only data of a certain date range, and then only Texas data. We'll build the filters one at a time so we can save the resulting objects separately.

### Filter by date

1.  Create a new section with headlines and text that denote you are filtering the data to Texas and since Jan. 1, 2010.
1.  Create a chunk and filter your `leso_control` data to get rows with a `ship_date` of 2010-01-01 or later.
2.  Save the result into a new tibble called `leso_dated`.
3.  Print out the new tibble `leso_dated`.
