...
- Update for Quarto
  - code-fold
- How do I deal with most No. 1 hits not including the Beatles Get Back collaboration.
- Do I need the lubridate library?
  - I think we do want to create `year` or something like that so there is experience for weather.
:::
...
::: callout-important
The data outputs in this book may differ from what you get since the data is updated with some regularity. This is especially true in videos and gifs.
:::

This chapter continues the Billboard Hot 100 project. In the previous chapter we downloaded, imported and cleaned our data. We'll be working in the same project.
...
-   **New for Fa23?** Appearances per year for a specific artist.
...
This will give you the website view of your page, but we need to add it to the navgation.

1. Open your `quart.yml` file
1. After the line that you adde for the cleaning notebook, add another new line (with the same indenting) and put in the name of your analysis file. It should be `- 02-analysis.qmd`.
...
So, what was your guess or this one? A little bit of history in that answer ... The Weeknd's *Blinding Lights* passed Imagine Dragon's *Radioactive* some time in 2021, but then Glass Animals' *Heat Waves* eclipsed that in 2022.
...
This would be a good time to again **Render** your notebook so you can see what the output will look like.
...
We haven't talked about filtering dates, so let me tell you this: You can use filter operations on dates just like you do any other text. This will give you rows *after* the last day of 2017.
