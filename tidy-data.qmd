...
2.  Create `data-original` and `data-processed` folders as we normally do.
...
download.file("https://docs.google.com/spreadsheets/d/e/2PACX-1vRCGayKLOy-52gKmEoPOj3ZKnOQVtCiooSloiCr-i_ci27e4n1CMPL0Z9s6MeFX9oQuN9E-HCFJnWjD/pub?gid=1456715839&single=true&output=csv", "data-original/candy.csv", mode = "wb")
```
...
#| message: false
#| warning: false
candy_raw <- read_csv("data-original/candy.csv") |> clean_names()
...
-   **`names_to=`** allows you to name the new column filled by the column names. *For our candy data we want to name this "color" since that's what that variable will contain, the color names.*
...
-   I want you to have a good title, subtitle, axis names, byline and source. Make sure a reader has all the information they need to understand what you are communicating with the chart, like the size of the bag (1.69 oz) and number of the packages considered. See the [Chart production tips](charts-tips.qmd) chapter for more detail. Do your best to make a complete chart.
...
When you get to the Publish & Embed window, I want you to

1. click the **Publish Now** button.
2. Once the URL is shown to you, click on the **For sharing** option and put that URL in text in your notebook so I can find it for grading.
...
candy_box_color <-
  candy_raw |> # <1> 
  filter_out(when_any(box_code == "130FXWAC0336", box_code == "1524976SE")) |> # <2>
...
2.  Filtering out some rows where there were only one or two bags. Not shown here is the research to find their values.
...
2.  Publish Posit Connect Cloud. Include the published link at the top of your notebook.
