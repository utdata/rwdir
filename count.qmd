...
We need some data to work on here. These are wells drilled in Hays, Travis and Williamson counties, which I then filter to recent wells drilled between 2020 and 2023. I also add a `yr` value based on the drilling date.
...
Each row here is a well drilled into the ground. The variable include details like where, who, when, etc. A couple of variables we'll pay attention to ...
...
We're counting the number of rows that have each proposed use.

### The beauty of tabyl

`tabyl()` works like count, but it also adds the percentage share by default. While the result looks physically different in your notebook compared to our tidyverse outputs, it is still a data frame that works in similar ways, as it can be piped, etc. They look the same when rendered.
...
Do note the `percent` column is still `<chr>` and would have to be converted to a number to do math or sorting properly.

### Two-way tabyls

Likd with `count()`, you can group by more than one value with `tabyl()`. We'll do this to get the proposed use type by year.
...
This might be a little confusing, but the `adorn_totals()` function can be used on any data frame, not just tabyls. Here I've taken the first few rows and columns of the starwars dataset and added `adorn_totals()`. See that it adds the totals of the included rows when they are numbers, and uses `-` when they are not numbers.
...
OK, this definitely gets confusing, but we can also get percentages for two-way tabyls.

Note that by default these are the **percentages within each ROW**, so of all the "Closed-Loop Geothermal" wells drilled between 2020 and 2023, 39.3% of them were in the year 2023.
...
What we really want is the percentage of wells were Closed-Loop Geothermal vs Domestic WITHIN a year.
...
Now we can see that the percentage of wells drilled for "Domestic" use have generally grown.
...
So here is a use case: I want to "see" how the percentage of water wells drilled for domestic use has changed year over year. Using `tabyl()` I can easily get the percentages within each year and then plot them.
...
wells_by_yr_type <- wells |> 
  ...
  pivot_longer(-proposed_use_clean, names_to = "yr", values_to = "percentage") |> # <4>
  mutate(percentage = parse_number(percentage)) # <5>

wells_by_yr_type
```

1. Build our tabyl using two variables, just like above
2. Switches our values to percentages based on the column
3. Converts to a friendly "* 100" percentage
4. Here we pivot_longer to make a data frame more friendly for ggplot
5. Because our "percentages" are text, we need to convert them to a number
