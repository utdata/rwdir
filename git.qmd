...
We'll use the `quarto publish` Terminal command to render and send files to our gh-pages branch in Github. This uses the same technology as the **Publish** button we use for Posit Connect Cloud in other parts of this book, but we'll use the terminal command to get our files to Github.
...
::: callout-tip
We are adding the `gh-pages` part to `quarto publish gh-pages` to pre-pick where we want to send the published files. You can also just `quarto publish` and then pick either Github Pages or other destinations as you see fit.
:::
...
I recommend just using a copy of my [.gitignore file from this book](https://github.com/utdata/rwdir/blob/gh-pages/.gitignore). It is best to create this file BEFORE YOU START COMMITTING FILES into your repo.
...
2.  Go the [.gitignore file from this book](https://github.com/utdata/rwdir/blob/gh-pages/.gitignore) and then copy/paste the contents into your newly-created file. Save and close.
