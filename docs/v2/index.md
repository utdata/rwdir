...
This work is written in [bookdown](https://bookdown.org/) and the source is available on [Github](https://github.com/utdata/rwdir). The text is under **constant revision**. The first version is somewhere in the commit history, but here are previous editions:
