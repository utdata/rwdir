...
Following the Chronicle's reporting, the Texas Legislature in 2017 [unanimously banned](https://www.chron.com/news/houston-texas/houston/article/Legislature-unanimously-approves-bill-designed-to-11134046.php) using a target or benchmark on how many students a district or charter school enrolls in special education.

**We want to know if there has been a change in the percentage of special education students in each school district since the law was passed and the policy was dropped. How many districts are above the 8.5% benchmark before and after the changes?**
...
1. In RStudio, choose File > New Project. **Choose Next Directory** at the next step and then find the folder you just moved.
...
There is a lot there to take in there. Here is where you end up:

- You have nine data files for each year and one reference file imported.

## Merging data together

OK, so we have nine different yearly files. Wouldn't it be a lot easier if these were ONE thing? Indeed, we can **merge** these files together by stacking them on top of each other.
...
Here's an image representation of the concept. (Note here that identical rows in both data sets remain).
...
Let's demonstrate just by building it.

1. Start a new section on your R Markdown document and note you are merging data
1. Add a chunk with the following and running it
...
Now **edit the chunk** to save the result do those things:
...
What joins do is match rows from two data sets that have column of in-common values, like an ID. (The `district` column in our case). Columns from the second data set will be added based on where the ID's match.

How much data you end up with depends on the "direction" of the join.

- An **inner_join** only keeps matching rows from both tables. If there are records in either table where the IDs don't match, then they are dropped.
- A **left_join** will keep ALL the rows of your first table, then bring data over from the second table when the IDs match. If there isn't a match in the second table, then new values will be blank.
...
- A **full_join** keeps rows from both tables, joining columns when they match.

Here's two common ways to think of this visually.

![](images/joins-description.png)

In the above table, The orange represents data that remains after the join.

![](images/join_types.png)

In this example above, they use a table to represent the changes. Only two rows "match" in each table so you can see what is left blank.

### Joining our reference table

In our case we are going to start with our `dref` data and then use an **inner_join** so join all the yearly data values. We're doing it in this order so the `dref` values are listed first.
...
I'm showing a glimpse here so you can see all the columns have been added.
...
##  1 234902   CANTON ISD       2014       2067        140          6.8 BELOW     
##  2 125905   PREMONT ISD      2015        504         49          9.7 ABOVE     
##  3 246904   GEORGETOWN ISD   2018      11508       1144          9.9 ABOVE     
##  4 009901   MULESHOE ISD     2016       1467        166         11.3 ABOVE     
##  5 031909   POINT ISABEL ISD 2019       2349        221          9.4 ABOVE     
##  6 232903   UVALDE CISD      2020       4236        435         10.3 ABOVE     
##  7 166904   ROCKDALE ISD     2020       1482        128          8.6 ABOVE     
##  8 070903   ENNIS ISD        2016       5799        613         10.6 ABOVE     
##  9 070903   ENNIS ISD        2021       5816        649         11.2 ABOVE     
## 10 108912   LA JOYA ISD      2021      26618       2885         10.8 ABOVE     
## # … with 20 more rows
```
