...
The concept of the pipe was first introduced by tidyverse developers in 2014 in a package called [magrittr](https://magrittr.tidyverse.org/). They used the symbol ` %>% ` as the pipe. It was so well received the concept was written directly into base R in **2021**, but using the symbol `|>`. Even Hadley Wickham is [rewriting R for Data Science](https://r4ds.hadley.nz/workflow-pipes.html#magrittr-and-the-pipe) to use the base R pipe `|>` by default. You can configure which to use in RStudio.
