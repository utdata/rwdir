...
> Note that you use "quotes" around the package name when you are installing, but you DON'T need quotes when you load the library.
...
Now that we know how to find our data, we next need to assign it to an **R object** so it can be named thing we can reuse it. We don't want to re-import the data every time we use the data.
...
We are starting with the object `hot100`, but then we follow it with ` |> `, which is called a pipe. The pipe is concept that allows us to take the **result** of an object or function and pass it into another function. Think of it like a sentence that says **"AND THEN" the next thing**.

It might look like there are no arguments inside `glimpse()`, but what we are actually doing is passing the `hot100` tibble into it like this: `glimpse(hot100)`.
...
The concept of the pipe was first introduced in 2014 as ` %>% ` by the tidyverse folks in 2014 with a package called [magrittr](https://magrittr.tidyverse.org/). It was so well received the concept was written directly into base R in 2011, but using the symbol `|>`. Even Hadley Wickham is [rewriting R for Data Science](https://r4ds.hadley.nz/workflow-pipes.html#magrittr-and-the-pipe) to use the base `|>` pipe by default. You can configure which to use in RStudio.

(This book will be rewritten to migrate from ` %>%` to`|>`, but it might take a while to catch all references to ` %>% `. For now, assume they are interchangeable.)
...
##  1st Qu.: 23.00   1st Qu.: 13.00   1st Qu.: 4.000   1st Qu.:1974-06-13  
##  Median : 47.00   Median : 38.00   Median : 7.000   Median :1990-04-17  
##  Mean   : 47.58   Mean   : 40.95   Mean   : 9.168   Mean   :1990-04-17  
##  3rd Qu.: 71.00   3rd Qu.: 65.00   3rd Qu.:13.000   3rd Qu.:2006-02-19  
