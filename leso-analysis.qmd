...
-   *How many total "controlled" items are in the hands of Texas agencies, and what are they all worth?* We'll summarize all the controlled items only to get the total quantity and total value of everything.
-   *How many total "controlled" items does each agency have and how much is it all worth?* Which agency got the most stuff?
    -   *How about local police agencies? What is the total quantity and value* I'll give you a list.
-   *What specific "controlled" items does each agency have and how much are they worth?* Now we're looking at the kinds of items.
...
5. Save the notebook as `02-analysis.qmd`.
3. Remove the boilerplate text.
1. After your metadata, add a markdown section that notes the goals of the notebook.
2. Copy 'n' paste the Questions to answer listed above, and formulate them as a list so you can refer back to them.
...
When I am querying my data, I start by envisioning what the result should look like. Let's take the first question: **How many total "controlled" items are in the hands of Texas agencies, and what are they all worth?**
...
1. Write a Data Takeaway sentence in your notebook summing up the totals in a way that you could drop into a story. Do note that since this is a point in time report, these are only items given since 2010 that are still with the agency. It's possible they received other items and have returned them to the Department of Defense for disposal.
...
OK, your next question is this: **For each agency, how many things do they have and how much is it all worth?**
...
aber_sum_quan <- aber_tots |> pull(summed_quantity)
aber_sum_tots <- aber_tots |> pull(summed_total_value)
```
...
If we look at the `quantity` column there we can eyeball all the rows and/or add them on a calculator . Our answer should match our `summed_quantity` in the summary table: `r aber_sum_quan`.

If we add up the `total_value` rows then we should end up with the same number as `summed_total_value` above: `r aber_sum_tots`.
...
Our second quest had a second part: **How about local police agencies? What is the total quantity and value?**
...
When we talked about filtering with the Billboard project, we discussed using `when_any()` as an "OR" function within filter. If we were to apply that logic here, it would look like this:

``` r
# Just for explanation. Don't add to notebook.
data |> 
  filter(when_any(column_name == "Text to find",
                  column_name == "More text to find")
  )
```
...
Our question is this: **What specific "controlled" items does each agency hanve and how much are they worth?**
...
1.  Publish your changes to Posit Connect Cloud and include the link to your project in your index notebook so I can bask in your glory.
