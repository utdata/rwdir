...
For this analysis we only want to look at "controlled" items, and only those items given to Texas agencies. We'll do some filtering to get to that data.
...
We can see from this result that there are `r leso |> filter(control_type == F) |> nrow()` items that are not controlled, and `r leso |> filter(control_type == T) |> nrow()` that are. When we write our filter, we should end up with `r leso |> filter(control_type == T) |> nrow()` observations, which is only the `TRUE` ones.

3.  Start with your `tx` data, but then filter it to `control_type == TRUE`.
4.  Save the result into a new tibble called `leso_c_tx`.
...
Since we want to initially look at just Texas data, we'll make a new object with just that. Remember, you want to do each step and run it to make sure it is working.

1.  Create a new section with headlines and text that denote you are filtering the data to Texas.
2.  Create the code chunk and start your filter process using the `leso` tibble.
3.  Use `filter()` on the `state` column to keep all rows with "TX".
4.  Edit the chunk to put all that into a new object called `tx`.
...
How do you know if it worked? Well the first column in the data is the `state` column, so they should all start with "TX". We are also down to `r nrow(leso_c_tx)` rows.
...
leso_c_tx |> 
  ...
  select(agency_name, quantity)
```

If we look at the `quantity` column there we can eyeball all the rows and add them on a calculator. Our answer should match our `summed_quantity` in the summary table. We're good!
