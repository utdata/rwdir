...
This chapter won't have data past 2025.
:::
...
hot100 <- hot100 |> filter(year(chart_date) < 2026)
```
...
::: callout-warning
Remember, your data is more current than what is in this book, so results may be slightly different.
:::
...
What we get in return is a **summarize**d table that shows all 11,000+ different performers that have been on the charts, and the **n**umber of rows in which they appear in the data.
...
We've printed 11,000+ rows of data into our notebook when we really only wanted the Top 10 or so. You might think it doesn't matter, but your knitted HTML file will store all that data and can make it a big file (as in hard drive space), so I try to avoid that when I can.
...
2. **CHECK THE FINDINGS AND EDIT IT** based on your results.
...
## get top row name
tp_p1_p <- tp_p1 |> pull(performer)

## get top row value
tp_p1_n <- tp_p1 |> pull(n) |> scales::comma()

## same values for 2nd place
tp_p2 <- tp |> slice(2)
tp_p2_p <- tp_p2 |> pull(performer)
tp_p2_n <- tp_p2 |> pull(n) |> scales::comma()
...
tp_p2_p
tp_p2_n
tp_d_diff <- tp_p1_n |> parse_number() - tp_p2_n |> parse_number()
```

```md
Data Takeaway: `r tp_p1_p` has more solo appearances on the Billboard Hot 100 than any other artist in history with `r tp_p1_n` appearances through `r mx_d`. That's `r tp_d_diff` more than `r tp_p2_p`, who has `r tp_p2_n`. This does not include collaborations with other artists.
```
...
| Operator                                     | Definition               |
|:---------------------------------------------|:-------------------------|
| x [**\<**]{.code_em} y                       | Less than                |
| x [**\>**]{.code_em} y                       | Greater than             |
| x [**==**]{.code_em} y                       | Equal to                 |
| x [**\<=**]{.code_em} y                      | Less than or equal to    |
| x [**\>=**]{.code_em} y                      | Greater than or equal to |
| x [**!=**]{.code_em} y                       | Not equal to             |
| x [**%in%**]{.code_em} c(y,z)                | In a group               |
| [**is.na(**]{.code_em}x[**)**]{.code_em}     | Is NA (missing values)   |
| [**!is.na(**]{.code_em}x[**)**]{.code_em}    | Is not NA                |
...
2. **CHECK AND EDIT the takeaway** as your answers might be different since you have newer data.
...
Now you have the answers to the song (or songs) with the most weeks at No. 1 with a logical cutoff. If you add to the data later, that logic will still hold and not cut off arbitrarily at a certain number of records.

### Data Takeaway: Longest at No. 1

1. Add this data takeaway to your notebook:
2. **CHECK AND REWRITE** if necessary.
...
```md
Data Takeaway: The song "`r no1_ttl`" by `r no1_per` has spent the most time at the top of the Billboard Hot 100 through `r mx_d` with `r no1_appr` appearances.
```
...
So, that makes this takeaway a bit more nuanced.

1. Add the takeaway below to your notebook.
2. **CHECK AND EDIT** if necessary.
...
1. Check, edit and add to your notebook.
...
Not to add confusion, but the Tidyverse folks [recently introduced](https://tidyverse.org/blog/2026/02/dplyr-1-2-0/) a variation of filter() called [`filter_out()`](https://dplyr.tidyverse.org/reference/filter.html). Think of it like this:

- If you are looking to define things to keep, use `filter()`.
- If you are looking for things to remove, use `filter_out()`.

The difference between the two functions is how they handles `NA` values. If you `filter()` for specific values, it will not include NA values. If `filter_out()` for specific values, it will keep NAs.

Think of it like this:

- You have a sea full of fish, but some of them you don't know the color.
- If you filter _for_ orange fish `filter(fish, color == "orange")`, you'll only keep the orange fish. All the other fish are gone, including those where you didn't know the color.
- If you filter _out_ the orange fish `filter_out(fish, color == "orange")`, all the orange fish are gone, and you keep all the other fish, even those where you don't know the color.

## Complex filters

You can combine filters in different ways to really target which rows to keep. For these I want you to play around a bit.

1.  Copy each of the examples below into your notebook, **but change the title and/or performer** to songs or artists you like.
...
If you want an **or** filter, then you wrap your conditions inside a `when_any()` function.

This shows songs by Adam Sandler OR Alabama Shakes.

```{r filter-or}
hot100 |> 
  filter(when_any(performer == "Adam Sandler", performer == "Alabama Shakes")) |> 
  select(chart_date:performer)
```
...
hot100 |> 
  filter(
    when_any(performer == "Taylor Swift", performer == "Drake"), # <1>
    current_rank == 1
  ) |> 
  distinct(current_rank, title, performer)
```

1. In this case we are first finding rows "when any" of the performer conditions is true ... for Taylor or Drake. Then the current rank rule is added.
...
Let's take that last one and then "filter out" the songs that include Drake.

```{r}
hot100 |> 
  filter(str_detect(performer, "2 Chainz")) |> 
  distinct(performer, title) |> 
  filter_out(str_detect(performer, "Drake"))
```

We've gone from 40 songs with "2 Chainz" down to 34 songs with "2 Chainz" but without "Drake".
...
3. **Read through the text and many necessary updates based on your newer data!**
3. Then add your own data takeaways at the end.

``` md
## Summary

`r tp_p1_p` has had more solo appearances on the Billboard Hot 100 chart than any other artist, according to an analysis of chart data from Billboard Magazine through `r mx_d`. Her `r tp_p1_n` solo appearances tops `r tp_p2_p` with `r tp_p2_n`.

> *You should update the date above and any other findings based on your newer data.

The chart has been published weekly since August 1958 and this analysis works from an [archive of those charts](https://github.com/utdata/rwd-billboard-data) managed by Christian McDonald, which is updated weekly.

Other findings from the data analysis include:

- The song "`r frst_ttl`" by `r frst_per` has appeared `r frst_appr` times on the Hot 100, more than any other song through `r year(mx_d |> mdy())`.
- "`r no1_ttl`" by `r no1_per` has spent more time at the top of the Hot 100 than any other songs in history at `r frst_appr` weeks.
- The Beatles have the most No. 1 hits with 19, but they also have a 20th top song -- "Get Back" -- that is a collaboration with Billy Preston. Most other artists with double-digit top hits aren't making new music, though Taylor Swift is climbing fast.
...
2.  Publish your changes to Connect Cloud and include the link to your project in your index notebook so you can bask in your glory.
3.  Zip your project folder. (Or export to zip if you are using posit.cloud).
4.  Upload to the Canvas assignment.

::: callout-important
To be clear, it is your zipped project I am grading. The Connect Cloud link is for convenience.
:::
