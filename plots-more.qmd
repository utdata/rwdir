...
This is the chart we hope to build:
...
- BEFORE printing that object, I save the plot with `ggsave()` with a path and name. Don't print out the object first or it will save at a weird size. Following R convention, I save plots into a folder called `figures/`. (You might need to create a "figures" folder first!)
- I then use Markdown to print the graphic to the notebook as an image using using this syntax:  `![Figure name](figures/image-name.png)`.
...
Now, to print the image in our notebook we use a syntax similar to a hyperlink, but start it with an exclamation point. `![Figure name](path-to-image.png)`. However, since our chart has a headline, we leave the name out of the square brackets.

For the chart we just made, I would add this after the code chunk:
...
It might not look very different here, but in my experience it is the most consistent method to get a nice display. I really only bother with all this if I'm having trouble getting everything to fit or I need to use the image outside of my notebook.
