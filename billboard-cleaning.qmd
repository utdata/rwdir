...
5.  Make sure your `_quarto.yml` file is saved.
...
``` text
trying URL 'https://github.com/utdata/rwd-billboard-data/blob/main/data-out/hot100_assignment_current.csv?raw=true'
Content type 'text/plain; charset=utf-8' length 18663203 bytes (17.8 MB)
==================================================
downloaded 17.8 MB
```
...
hot100_raw <- read_csv("data-raw/hot100_assignment.csv")
```
...
hot100_raw <- read_csv("data-raw/hot100_assignment.csv") |> clean_names()
...
Sorting data is not a particularly difficult concept to grasp, but it is one of the [Basic Data Journalism Functions](https://www.youtube.com/playlist?list=PLggPkq8Rt2UEHGtgsvu__zheKF4Jjuxfa), so watch this video:
...
We will use another function from the readr package called `write_rds()` to create our exported file. We'll save the data into the `data-processed` folder we created earlier. We are avoiding our data-raw folder because **"Thou shalt not change original data"** even by accident.
...
Remember, we are saving into the data-processed folder because we never export into data-raw. We are naming the file starting with `01-` to indicate to our future selves that this output came from our first notebook. We then name it, and use the `.rds` extension.
