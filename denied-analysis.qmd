...
This is the second of of a two-chapter project. Now that we have assembled and cleaned our data, we need to answer these questions in our followup to the [Denied](https://www.houstonchronicle.com/denied/) series:
...
#| code-fold: true
#| code-summary: "You won't need this because I believe in you"
...
BTW, if you wanted to make that chart in [datawrapper](https://academy.datawrapper.de/article/20-how-to-create-a-column-chart), you could use the same tibble data.
...
1. We added `ylim()` here because the default didn't start the y axis at zero. `ylim` is a shortcut for `scale_y_continuous(limits = c(0,1000))`.
...
- [`bind_rows()`](https://dplyr.tidyverse.org/reference/bind.html) allows us to stack similar data into a single object. 
- [`inner_join()`](https://dplyr.tidyverse.org/reference/mutate-joins.html) and its siblings allow us to add columns based on a common values in the data sets.
- We utilized [`count()`](https://dplyr.tidyverse.org/reference/count.html) which is a shortcut for grouping and counting rows of data.
