...
- Choose the **Student Information** data set and choose **Continue**.
  ...
  - Also choose **Student Enrollment by Program** ^[The name changes to "Students by Instructional Programs" in later years.]
  - Click **Download**.

This downloads a file called `DSTUD.csv` or `DSTUD.dat`. As I downloaded them, I changed the name to include the year and `.csv` extension and put them in the `data-original/` folder.
...
We've noted that the dstud files don't have a **district name** within them, so we don't know what this district id means. Within the TAPR data download tool, there is another file called **District Reference** that has the district ID, names and other information we need. I downloaded the 2022 version ^[Later District Reference files from the TEA are missing some variables.] as `DREF_22.csv`. Let's look at it:
...
read_csv("data-original/DREF_22.csv") |> head()
```
