...
- Import multiple files at once and bind them with `map_dfr()`
...
1. Read [Part 1](https://www.houstonchronicle.com/denied/1/) of the original Denied series, at least to the heading "A special child." Pay attention to parts about the "Performance-Based Monitoring Analysis System."
...
In our `raw-data/` folder we have 10 years of data from the 2012-2013 school year to the 2020-2022 school year. The "long" way to do this would be importing each file individually, selecting the columns we need and then saving them as objects. We could then stack them on top of each other with the [`bind_rows()`](https://dplyr.tidyverse.org/reference/bind_rows.html) function.

But can use some libraries available in tidyverse -- purrr and readr -- to import and bind together all the DSTUD files at one time. This only works if the data are structured in the same way.

We'll do this in two steps:

- We start by building a list of our files
- Then we'll use a function to loop over the list and import our files, choosing only the columns we want at the same time.

#### Building our files list

We'll use a function from the `fs` library called `dir_ls()` to capture a list of the files we want.
...
4. Add text noting we are building our list of files to import.
5. Run the code.

I explain the code below.

```{r}
dstud_files <- dir_ls( # <1>
  "data-raw", # <2>
  regexp = "DSTUD" # <3>
)
...
1. The first line starts with the new object we are creating to hold our list of data. It ends with our function, `dir_ls()`, which lists files in a directory as a vector.
2. The first argument of `dir_ls()` is the path to where you want to look. We want to look in the `data-raw` folder. Without this argument, it would look in our main projet folder.
3. The second optional argument is a regular expression (or regexp) search query to capture specific files based on the file name. This gets us just the DSTUD files and not the DREF file we need later.
4. We're just printing out the resulting object to make sure we got what we wanted.

::: callout-tip
## About regular expressions

Regular expressions are a pattern-matching syntax used in many programming languages. Above we searched for a simple word, but regex is much more powerful than that. You can [learn more about regular expressions here](https://docs.google.com/document/d/1DvAM4lnGJLefo9skD8GgM-_9S1BEhpjJfV86yhJavI0/edit?usp=sharing).
:::
...
Next we want to take that list and walk through it using using a "map" function called [`map_dfr()`](https://purrr.tidyverse.org/reference/map_dfr.html) which returns a data frame, binding the rows of each file together.

The idea here is the map_dfr function loops over the list, applying the arguments inside the function on each item. The end result is a single data frame.
...
2. We pipe into `map_dfr`, which will work on each file in turn, combining the results into a single data frame.
3. The first argumemt for map_dfr is the function we are going to use without our loop. We are using `read_csv()` since we are importing csv files.
4. This `.id = ` is an optional argument saves a new column called "source" with our file path, otherwise we won't know which row came from which file.
5. This `col_select =` is an optional argument for `read_csv` that allows us to choose which columns we want as we read them in.
6. We clean the names after importing them.
7. Here we are peeking at the top of the resulting object.
7. And then we peek at the bottom of the object.

::: callout-tip

### The col_select argument vs select()

This `col_select = ` argument we use above also works with a regular `read_csv()` function. It provides the same result as using `read_csv()` to import all the columns and the piping into a `select()` function to list the columns you want to keep or drop. It's yet another example showing multiple ways to accomplish the same result with R. Up to know I've preferred to use `read_csv() |> select()` because that allows you to see everything inside the data before choosing what to do with it. In our case above being able to pick just these few columns we need makes this much easier to deal with since the individual files have differences beyond some common columns.
...
dstud <- dstud_raw |> # <1>
  mutate(
    ...
    year = str_c("20", str_sub(source, 16, 17)) # <3>
  ) |> 
  select(!source) # <4>
...
3. The second mutate operation is more complicated. We are setting `year` to equal a "string combination" (the `str_c()` function) that pieces together the text "20" with characters from the `source` variable based on their position, which happen to be the numbers of the year. To do that we use `str_sub()` where the first argument is variable to look at (source), the starting position (the 16th character) and the ending position (the 17th one). This only works because all the file names are structured the same way.
...
2. The list of columns we need, using the `col_select =` argument to show we can.
...
- [`dir_ls()`](https://www.rdocumentation.org/packages/fs/versions/1.6.3/topics/dir_ls) from the [fs](https://www.tidyverse.org/blog/2018/01/fs-1.0.0/) package makes a list of file names in a folder. You can use the `regexp = ` argument to search for specific files to include based on a pattern.
- [`map_dfr()`](https://purrr.tidyverse.org/reference/map_dfr.html) Loops through a list of files with a data frame as a result. We used it with `read_csv()` to import multiple files and bind them together into a single data object.
