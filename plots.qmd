...
4. In you `index.qmd` file, remove the boilerplate and create a setup section that loads `library(tidyverse)`, like we do with every notebook. The `ggplot` package is a part of `tidyverse`, so when you load `tidyverse`, you'll load `ggplot`. We will also load `janitor`, which we will load to use the `clean_names()` function.
