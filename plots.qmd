...
Now that we know which data to apply, we have to make a choice on how we want to show the shape of that data on our plot, and we do that through **geometries**. We'll talk about different ones in a minute, but we'll start with plotting our data as points.
...
OK, now we are getting somewhere. We can **see** each penguin plotted on the chart, and we can generally see that as they get heavier (higher on vertical axis) their flippers are also longer (to the right on the horizontal axis.)
