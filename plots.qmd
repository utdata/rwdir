...
::: callout-warning
- Quarto draft is finished
- Go back and add goal plot for princess
:::

::: callout
The chapter is by Prof. Lukito with some updates by Prof. McDonald.
:::
...
As noted above, the data is measurements from a number or penguins, things like species, sex, bill length, etc.
...
1. **Edit** your code to add the following line of code.
...
In this case we start with the data AND THEN pipes it into `ggplot()` as the first argument. Prof. McDonald often does it this way. Scoundrel.
:::
...
In addition to geoms, there are other layers we can use to change or add to our plot.
...
Let's add a labels layer so we can describe our chart to our readers.
...
Note that there are several arguments within the `labs()` function:

- `title` gives us a headline
- `subtitle` is a readout under the headline.
- The `x` axis already had a label on our chart by default, but it was the name of the variable. Find for us, because we know what it is, but here we are replacing that with text more appropriate for a reader. The same with the `y` value we reset.
- The `color` and `shape` arguments are similar in that we are replacing the existing label above the legend, resetting those to capitalize "Species". We need to change the both so they are the same. (You might experiment by removing the `shape` argument and re-running it to see what happens.)
...
Folks have saved combinations of these arguments into **themes** that allow you to radicaly change the appearance of a plot with a single layer. Let's how that here.
...
This changes the grid and gridline colors to make the data pop a little better. There are other themes that create more radical change, some in their own packages. We'll play more later.
...
class <- read_csv(
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vQfwR6DBW5Qv6O5aEBFJl4V8itnlDxFEc1e_-fOAtBMDxXx1GeEGb8o5VSgi33oTYqeFhVCevGGbG5y/pub?gid=0&single=true&output=csv"
)
...
princess_data <- class |> 
  clean_names() |> #remember, this is from the janitor package!
...
::: callout-note
Complication alert: Categorical data can have [factors](https://r4ds.hadley.nz/factors.html), which are like an internal ordering system. Some categories, like months in a year, have an "order" that is not alphabetical.
:::
...
ggplot(
  princess_data,
  aes(x = reorder(princess, votes), y = votes) # this is the line you edit
) + 
  geom_col() +
  coord_flip()
```
