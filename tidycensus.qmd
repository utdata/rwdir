...
This chapter shows an example of how to find data fro the American Community Survey using [Census Reporter](https://censusreporter.org/) and the to get that data using the [tidycensus](https://walker-data.com/tidycensus/index.html) package.
...
- This year's [NICAR lesson on tidycensus](https://nicar.r-journalism.com/2024/) and the [Github repo](https://github.com/r-journalism/nicar-2024-tidycensus?tab=readme-ov-file) for it
...
The goal here is to find the percentage of women with degree higher than high school. To do this we have to add together all the different degree types, then divide by all women.
...
We are using Census Reporter to find the variables we need, looking at [Table B15002: Sex by Educational Attainment](https://censusreporter.org/tables/B15002/). (I'll explain some other time why I started here.)
...
moe_check <- college_vars |> 
  mutate(
    moe_share = ((moe / estimate) * 100) |> round(1), # calc moe share
...
If the moe_flag was TRUE anywhere then we might want to use that number. We are find with these numbers above so I'm going to drop the `moe` column in the next section.
...
college_calcs <- college_wide |> 
  mutate(
    prc_hs = ((fem_hs / fem) * 100) |> round(1),
    sum_deg = fem_as_d + fem_ba_d + fem_ma_d + fem_ps_d + fem_ph_d,
    prc_deg = ((sum_deg / fem) * 100) |> round(1)
  )
