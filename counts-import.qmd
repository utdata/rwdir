...
3. The first option available (and perhas the only) should be Quarto Pub. Hit **Return** to choose that.
...
<!--
PROBABLY NEED TO UPLOAD TO VIMEO
--> 

{{< video images/bb-quarto-pub.mp4 >}}

Well done.

## Review of what we've learned so far

Most of this lesson has been about importing and cleaning data, which can sometimes be the most challenging part of a project, depending on the circumstances. Here we were working with well-formed data, but we still used quite a few tools from the tidyverse like readr (read_csv, write_rds) and dplyr (select, mutate).
